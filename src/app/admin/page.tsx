    ...
    <div className="admin-layout">
      ...
      <aside className="admin-sidebar">
        ...
        <nav className="admin-nav">
          ...
          <Link href="/admin/audit" className="admin-nav-item">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <main className="admin-main">
        <header className="admin-header">
          <div className="admin-header-left">
            <h1 className="admin-page-title">帖子管理</h1>
            <span className="admin-page-subtitle">共 {posts.length} 篇帖子</span>
          </div>
        ...
        <div className="admin-content">
          <div className="admin-card">
            <div className="admin-card-header">
              <h2 className="admin-card-title">所有帖子</h2>
              <span className="admin-card-badge">{posts.length}</span>
            </div>
            <div className="admin-table-wrapper">
              <table className="admin-table">
                ...
                <tbody>
                  {posts.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="admin-table-empty">暂无帖子</td>
                    </tr>
                  ) : (
                    posts.map((post) => (
                      <tr key={post.id}>
                        <td>
                          <Link href={postPath(post.slug)} className="admin-post-link">
                            {post.title}
                          </Link>
                        </td>
                        ...
                        <td>
                          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                            {post.pinned && (
                              <span className="admin-status-badge admin-status-pinned">置顶</span>
                            )}
                            {post.featured && (
                              <span className="admin-status-badge admin-status-featured">加精</span>
                            )}
                            {!post.pinned && !post.featured && (
                              <span className="admin-status-badge">普通</span>
                            )}
                          </div>
                        </td>
                        <td>
          ...
          <div className="admin-tip">
            ...
            <span>
              提升管理员：在服务器执行{" "}
              <code>npm run db:promote-admin -- 你的邮箱</code>
            </span>
          </div>
        </div>
      </main>
